...
for y, line in enumerate(data):
    for x, character in enumerate(line):
        if character == '#':
            grid[(0, y, x)] = character # storing each # in grid as tuple of coords

for _ in range(6): # 6 cycles
    active = collections.Counter() # counter dict

    for (z,y,x), _ in grid.items(): # for loops for possible dimension coords
        for znum in (-1, 0 , 1):
            for ynum in (-1, 0 , 1):
                for xnum in (-1, 0, 1):
                    if znum == ynum == xnum == 0:
                        continue
                    active[(z+znum, y+ynum, x+xnum)] +=1
